...
export default function StepFive({ onNext }: { onNext: () => void }) {
  ...
  return (
    <div className="w-full flex flex-col gap-8 animate-fade-in select-none">
      ...
      <div className="bg-[#FBFBFB] border border-[#EAEAEA] rounded-[24px] p-6 w-full mt-4">
          ...
          {activeAttires.map((attire) => {
            ...
            return (
              <div
                ...
                className={`relative h-[380px] rounded-[20px] overflow-hidden cursor-pointer border-[4px] transition-all duration-300 group select-none ${
                  isSelected
                    ? "border-[#A855F7] shadow-[0_0_25px_rgba(168,85,247,0.6)] scale-[1.02]"
